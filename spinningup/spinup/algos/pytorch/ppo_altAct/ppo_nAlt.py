...
import gym
import time
import spinup.algos.pytorch.ppo_nAlt.core as core
...
class PPOBuffer:
    ...
    def store(
        self, obs, act, rew, val, logp, alt_obss, alt_acts, alt_rews, alt_vals="nan"
    ):
...
def ppo_nAlt(
    ...
    def update():
        ...
        for i in range(train_alt_pi_iters):
            ...
            loss.backward()
            torch.nn.utils.clip_grad_norm_(ac.pi.parameters(), grad_norm_clip / 2)
            pi_optimizer.step()
...
if __name__ == "__main__":
    ...
    shutil.copyfile(__file__, os.path.join(logger_kwargs["output_dir"], "ppo_nAlt.py"))
    shutil.copyfile(
