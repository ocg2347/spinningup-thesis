...
class CNPActor(nn.Module):
    def __init__(self, obs_dim, act_dim, hidden_sizes, encoder_output_size, activation):
        super().__init__()
        self.cnp = CNP_w_seperateSTD(in_shape=(obs_dim, act_dim),
...
class CNPActorMLPCritic(nn.Module):
    def __init__(self, observation_space, action_space, 
                 hidden_sizes=(64,64), encoder_output_size=1,
                 activation=nn.Tanh, use_time_input=True):
        super().__init__()
        obs_dim = observation_space.shape[0]
        # policy builder depends on action space
        policy_obs_dim = obs_dim+1 if use_time_input else obs_dim
        if isinstance(action_space, Box):
            self.pi = CNPActor(policy_obs_dim, action_space.shape[0], hidden_sizes, encoder_output_size, activation)
        else:
            raise NotImplementedError
